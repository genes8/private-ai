...
**Updated** The system now features a revolutionary three-mode provider architecture that replaces the single-provider configuration with flexible deployment options. The new embedding_source field and provider_mode property enable administrators to choose between Local (everything on Ollama), Hybrid (cloud LLM with local embeddings), and Cloud (everything on cloud provider) modes, providing unprecedented operational flexibility and deployment options.
...
- **Vision**: Uses local Ollama vision models
...
- **Vision**: Local Ollama vision models
...
- **Vision**: Uses cloud provider vision models
...
```mermaid
flowchart TD
Start([Application Startup]) --> LoadDB["Load AppConfig from Database"]
...
The pipeline ensures robust configuration management through multiple validation layers and graceful fallback mechanisms, now supporting three distinct provider modes.
...
- **Models**: All models from local Ollama
...
- **Vision Model**: Local Ollama vision model (qwen2.5vl:7b)
...
- **API Key**: Required for cloud provider access
- **Models**: Cloud provider models for chat, embeddings, and vision
- **Usage Tracking**: Actual token counts from cloud provider
- **Requirements**: Cloud provider must support embeddings API
...
- **Vision Model**: Select from available Ollama models
...
- **Vision Model**: Select from cloud provider models
...
**Updated** The system now includes enhanced troubleshooting guidance for three-mode configuration issues, embedding model configuration problems, scoring system problems, and rollback functionality:
