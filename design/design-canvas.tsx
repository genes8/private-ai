...
interface DCApi {
  state: DCState;
  section: (id: string) => Record<string, any>;
  patchSection: (id: string, p: Record<string, any> | ((prev: Record<string, any>) => Record<string, any>)) => void;
...
function DesignCanvas({ children, minScale, maxScale, style }: DesignCanvasProps) {
  ...
  React.Children.forEach(children, (sec) => {
    if (!sec || (sec as React.ReactElement).type !== DCSection) return;
    const sid = (sec as React.ReactElement<DCSectionProps>).props.id ?? (sec as React.ReactElement<DCSectionProps>).props.title;
    if (!sid) return;
    sectionOrder.push(sid);
    const persisted = state.sections[sid] || {};
    const abs: [string, React.ReactElement<DCArtboardProps>][] = [];
    React.Children.forEach((sec as React.ReactElement<DCSectionProps>).props.children, (ab) => {
      if (!ab || (ab as React.ReactElement).type !== DCArtboard) return;
      const aid = (ab as React.ReactElement<DCArtboardProps>).props.id ?? (ab as React.ReactElement<DCArtboardProps>).props.label;
    ...
    sectionMeta[sid] = {
      title: persisted.title ?? (sec as React.ReactElement<DCSectionProps>).props.title,
      subtitle: (sec as React.ReactElement<DCSectionProps>).props.subtitle,
  ...
  const api = React.useMemo<DCApi>(() => ({
    ...
    patchSection: (id: string, p: Record<string, any> | ((prev: Record<string, any>) => Record<string, any>)) => setState((s) => ({
      ...s,
...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }: {
  ...
  const saveT = React.useRef<number>(0);

  const lastPostedScale = React.useRef<number | undefined>();
  ...
  React.useEffect(() => {
    ...
    const onGestureChange = (e: Event) => {
      e.preventDefault();
      zoomAt((e as any).clientX, (e as any).clientY, (gsBase * (e as any).scale) / tf.current.scale);
    };
    ...
    const onPointerDown = (e: PointerEvent) => {
      const onBg = !(e.target as HTMLElement).closest('[data-dc-slot], .dc-editable');
    ...
    vp.addEventListener('wheel', onWheel, { passive: false } as any);
    vp.addEventListener('gesturestart', onGestureStart as any, { passive: false } as any);
    vp.addEventListener('gesturechange', onGestureChange as any, { passive: false } as any);
    vp.addEventListener('gestureend', onGestureEnd as any, { passive: false } as any);
...
function DCSection({ id, title, subtitle, children, gap = 48 }: DCSectionProps) {
  ...
  const artboards = all.filter((c) => c && (c as React.ReactElement).type === DCArtboard);
  const rest = all.filter((c) => !(c && (c as React.ReactElement).type === DCArtboard));
  const sec = (ctx && sid && ctx.section(sid)) || {};
  // Must match DesignCanvas's srcKey computation exactly (it filters falsy
  // IDs), or onDelete persists a srcKey that DesignCanvas never recognizes.
  const allIds = artboards.map((a) => ((a as React.ReactElement<DCArtboardProps>).props.id ?? (a as React.ReactElement<DCArtboardProps>).props.label) as string).filter(Boolean);
  const srcKey = allIds.join('\x1f');
  const hidden: string[] = sec.srcKey === srcKey ? (sec.hidden || []) : [];
  ...
  const byId = Object.fromEntries(artboards.map((a) => [(a as React.ReactElement<DCArtboardProps>).props.id ?? (a as React.ReactElement<DCArtboardProps>).props.label, a]));
      ...
      <div style={{ padding: '0 60px' }}>
        <div className="dc-sectionhead" style={{ paddingBottom: 36 }}>
          <DCEditable tag="div" value={sec.title ?? title}
            ...
            style={{ fontSize: 28, fontWeight: 600, color: DC.title, letterSpacing: -0.4, marginBottom: 6, display: 'inline-block' } as React.CSSProperties} />
          {subtitle && <div style={{ fontSize: 16, color: DC.subtitle }}>{subtitle}</div>}
        </div>
      </div>
      <div style={{ display: 'flex', gap, padding: '0 60px', alignItems: 'flex-start', width: 'max-content' }}>
        {order.map((k: string) => (
          <DCArtboardFrame key={k} sectionId={sid} artboard={byId[k] as React.ReactElement<DCArtboardProps>} order={order}
            label={(sec.labels || {})[k] ?? (byId[k] as React.ReactElement<DCArtboardProps>).props.label}
            onRename={(v: string) => ctx && ctx.patchSection(sid, (x: Record<string, any>) => ({ labels: { ...x.labels, [k]: v } }))}
            onReorder={(next: string[]) => ctx && ctx.patchSection(sid, { order: next })}
            onDelete={() => ctx && ctx.patchSection(sid, (x: Record<string, any>) => ({
...
async function dcExport(node: HTMLElement, w: number, h: number, name: string, kind: 'png' | 'html') {
  try { await document.fonts.ready; } catch {}
  const toDataURL = (url: string) => fetch(url).then((r) => r.blob()).then((b) => new Promise<string>((res) => {
    const fr = new FileReader(); fr.onload = () => res(fr.result as string); fr.onerror = () => res(url); fr.readAsDataURL(b);
  })).catch(() => url);
  ...
  const fontRules: { css: string; base: string }[] = [], pending: Promise<void>[] = [], seen = new Set<string>();
  ...
  for (const ss of document.styleSheets as any) {
    const base = ss.href || location.href;
    try { walk(ss.cssRules, base); } catch { if (ss.href) scrapeCss(ss.href); }
  }
  while (pending.length) await pending.shift();
  const fontCss = (await Promise.all(fontRules.map(async (rule) => {
    let out = rule.css, m: RegExpExecArray | null; const re = /url\((['"]?)([^'")]+)\1\)/g;
    while ((m = re.exec(rule.css))) {
      if (m[2].indexOf('data:') === 0) continue;
      let abs: string; try { abs = new URL(m[2], rule.base).href; } catch { continue; }
  ...
  const cloneStyled = (src: Node): Node => {
    if (src.nodeType === 8 || (src.nodeType === 1 && (src as HTMLElement).tagName === 'SCRIPT')) return document.createTextNode('');
    const dst = src.cloneNode(false);
    if (src.nodeType === 1) {
      const cs = getComputedStyle(src as HTMLElement); let txt = '';
      for (let i = 0; i < cs.length; i++) txt += cs[i] + ':' + cs.getPropertyValue(cs[i]) + ';';
      (dst as HTMLElement).setAttribute('style', txt + 'animation:none;transition:none;');
      if ((src as HTMLElement).tagName === 'CANVAS') try { const im = document.createElement('img'); im.src = (src as HTMLCanvasElement).toDataURL(); im.setAttribute('style', txt); return im; } catch {}
    }
  ...
  [clone, ...Array.from(clone.querySelectorAll('*'))].forEach((el) => {
    const bg = (el as HTMLElement).style.backgroundImage; if (!bg) return;
    let m: RegExpExecArray | null; const re = /url\(["']?([^"')]+)["']?\)/g;
    while ((m = re.exec(bg))) {
      const tok = m[0], url = m[1];
      if (url.indexOf('data:') === 0) continue;
      jobs.push(toDataURL(url).then((d) => { (el as HTMLElement).style.backgroundImage = (el as HTMLElement).style.backgroundImage.split(tok).join('url("' + d + '")'); }));
    }
  });
...
function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete }: DCArtboardFrameProps) {
  ...
  React.useEffect(() => {
    ...
    const off = (e: PointerEvent) => { if (!menuRef.current || !menuRef.current.contains(e.target as Node)) setMenuOpen(false); };
  ...
  const onGripDown = (e: React.PointerEvent) => {
    ...
    const homes = peers.map((el) => ({ el, id: (el as HTMLElement).dataset.dcSlot!, x: el.getBoundingClientRect().left }));
        ...
        <div className="dc-labelrow">
          ...
          <div className="dc-labeltext" onClick={onFocus} title="Click to focus">
            <DCEditable value={label} onChange={onRename} onClick={(e: React.MouseEvent) => e.stopPropagation()}
              style={{ fontSize: 15, fontWeight: 500, color: DC.label, lineHeight: 1 } as React.CSSProperties} />
          </div>
        </div>
